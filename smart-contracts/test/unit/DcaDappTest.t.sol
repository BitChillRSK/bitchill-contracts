...
contract DcaDappTest is Test {
    ...
    event DcaManager__TokenBalanceUpdated(
    ...
    //MockMocProxy
    event MockMocProxy__DocRedeemed(
        address indexed user,
        uint256 docAmount,
        uint256 btcAmount
    );
    ...
    function setUp() public virtual {
        ...
        emit AdminOperations__TokenHandlerUpdated(
            docTokenAddress,
    ...
    function depositDoc() internal returns (uint256, uint256) {
        vm.startPrank(USER);
        uint256 userBalanceBeforeDeposit = dcaManager.getScheduleTokenBalance(
            address(mockDocToken),
            SCHEDULE_INDEX
        );
        mockDocToken.approve(address(docTokenHandler), DOC_TO_DEPOSIT);
        vm.expectEmit(true, true, true, false);
        emit TokenHandler__TokenDeposited(
            address(mockDocToken),
            USER,
            DOC_TO_DEPOSIT
        );
        ...
        emit DcaManager__TokenBalanceUpdated(
            ...
            scheduleId,
            2 * DOC_TO_DEPOSIT
        ); // 2 *, since a previous deposit is made in the setup
        dcaManager.depositToken(
            address(mockDocToken),
            SCHEDULE_INDEX,
            DOC_TO_DEPOSIT
        );
        uint256 userBalanceAfterDeposit = dcaManager.getScheduleTokenBalance(
            address(mockDocToken),
            SCHEDULE_INDEX
        );
    ...
    function withdrawDoc() internal {
        vm.startPrank(USER);
        vm.expectEmit(true, true, true, false);
        emit TokenHandler__TokenWithdrawn(
            address(mockDocToken),
            USER,
            DOC_TO_DEPOSIT
        );
        ...
        uint256 remainingAmount = dcaManager.getScheduleTokenBalance(
            address(mockDocToken),
            SCHEDULE_INDEX
        );
    ...
    function createSeveralDcaSchedules() internal {
        ...
        for (uint256 i = 0; i < NUM_OF_SCHEDULES; ++i) {
            ...
            if (
                dcaManager.getMyDcaSchedules(address(mockDocToken)).length >
                scheduleIndex
            ) {
                userBalanceBeforeDeposit = dcaManager.getScheduleTokenBalance(
                    address(mockDocToken),
                    scheduleIndex
                );
            } else {
            ...
            uint256 userBalanceAfterDeposit = dcaManager
                .getScheduleTokenBalance(address(mockDocToken), scheduleIndex);
            assertEq(
                docToDeposit,
                userBalanceAfterDeposit - userBalanceBeforeDeposit
            );
            assertEq(
                purchaseAmount,
                dcaManager.getSchedulePurchaseAmount(
                    address(mockDocToken),
                    scheduleIndex
                )
            );
            assertEq(
                purchasePeriod,
                dcaManager.getSchedulePurchasePeriod(
                    address(mockDocToken),
                    scheduleIndex
                )
            );
    ...
    function makeSinglePurchase() internal {
        ...
        uint256 RbtcBalanceBeforePurchase = docTokenHandler
            .getAccumulatedRbtcBalance();
        IDcaManager.DcaDetails[] memory dcaDetails = dcaManager
            .getMyDcaSchedules(address(mockDocToken));
        vm.stopPrank();
        ...
        vm.startPrank(USER);
        uint256 docBalanceAfterPurchase = dcaManager.getScheduleTokenBalance(
            address(mockDocToken),
            SCHEDULE_INDEX
        );
        uint256 RbtcBalanceAfterPurchase = docTokenHandler
            .getAccumulatedRbtcBalance();
        vm.stopPrank();
        // Check that DOC was substracted and rBTC was added to user's balances
        assertEq(
            docBalanceBeforePurchase - docBalanceAfterPurchase,
            DOC_TO_SPEND
        );
        assertEq(
    ...
    function makeSeveralPurchasesWithSeveralSchedules()
        internal
        returns (uint256 totalDocSpent)
    {
        ...
        for (uint8 i; i < NUM_OF_SCHEDULES; ++i) {
            ...
            vm.startPrank(USER);
            uint256 schedulePurchaseAmount = dcaManager
                .getSchedulePurchaseAmount(
                    address(mockDocToken),
                    scheduleIndex
                );
            uint256 schedulePurchasePeriod = dcaManager
                .getSchedulePurchasePeriod(
                    address(mockDocToken),
                    scheduleIndex
                );
            vm.stopPrank();
            uint256 fee = feeCalculator.calculateFee(
                ...
                schedulePurchasePeriod
            );
            uint256 netPurchaseAmount = schedulePurchaseAmount - fee;

            for (uint8 j; j < numOfPurchases; ++j) {
                vm.startPrank(USER);
                uint256 docBalanceBeforePurchase = dcaManager
                    .getScheduleTokenBalance(
                        address(mockDocToken),
                        scheduleIndex
                    );
                uint256 RbtcBalanceBeforePurchase = docTokenHandler
                    .getAccumulatedRbtcBalance();
                bytes32 scheduleId = dcaManager.getScheduleId(
                    address(mockDocToken),
                    scheduleIndex
                );
                vm.stopPrank();
                ...
                vm.startPrank(USER);
                uint256 docBalanceAfterPurchase = dcaManager
                    .getScheduleTokenBalance(
                        address(mockDocToken),
                        scheduleIndex
                    );
        ...
        vm.prank(USER);
        assertEq(
            docTokenHandler.getAccumulatedRbtcBalance(),
    ...
    function makeBatchPurchasesOneUser() internal {
        ...
        vm.prank(USER);
        uint256 userAccumulatedRbtcPrev = docTokenHandler
            .getAccumulatedRbtcBalance();
        // vm.prank(OWNER);
        ...
        for (uint8 i; i < NUM_OF_SCHEDULES; ++i) {
            ...
            scheduleIndexes[i] = i;
            vm.startPrank(OWNER);
            purchaseAmounts[i] = dcaManager
            .ownerGetUsersDcaSchedules(users[0], address(mockDocToken))[i]
                .purchaseAmount;
            purchasePeriods[i] = dcaManager
            .ownerGetUsersDcaSchedules(users[0], address(mockDocToken))[i]
                .purchasePeriod;
            scheduleIds[i] = dcaManager
            .ownerGetUsersDcaSchedules(users[0], address(mockDocToken))[i]
                .scheduleId;
            vm.stopPrank();
        }
        for (uint8 i; i < NUM_OF_SCHEDULES; ++i) {
            ...
            emit TokenHandler__RbtcBought(
                ...
                0,
                0
            ); // Never mind the actual values on this test
        }
        vm.expectEmit(true, true, true, false);
        ...
        assertEq(
            postDocTokenHandlerBalance - prevDocTokenHandlerBalance,
            totalNetPurchaseAmount / BTC_PRICE
        );

        vm.prank(USER);
        uint256 userAccumulatedRbtcPost = docTokenHandler
            .getAccumulatedRbtcBalance();
        ...
        assertEq(
            postDocTokenHandlerBalance2 - postDocTokenHandlerBalance,
            totalNetPurchaseAmount / BTC_PRICE
        );
    }
}
