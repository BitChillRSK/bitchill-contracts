...
contract ComparePurchaseMethods is Test {
    ...
    function compareSinglePurchase() private {
        ...
        if (mocGasCostRbtc > uniGasCostRbtc) {
            ...
            uint256 extraCostUsd = (extraCostRbtc * btcPrice) / 1e13; // Divide by less to preserve more decimals
            console2.log("MoC is more expensive to operate:");
            console2.log("  Extra cost in rBTC (WEI):", extraCostRbtc);
            console2.log("  Extra cost in USD:", extraCostUsd);
        } else if (uniGasCostRbtc > mocGasCostRbtc) {
            uint256 extraCostRbtc = uniGasCostRbtc - mocGasCostRbtc;
            uint256 extraCostUsd = (extraCostRbtc * btcPrice) / 1e13;
        ...
        if (mocRbtcPurchased > uniRbtcPurchased) {
            ...
            uint256 extraUsd = (extraRbtc * btcPrice) / 1e13;
            console2.log("MoC provides better returns:");
            console2.log("  Extra rBTC gained (WEI):", extraRbtc);
            console2.log("  Extra value in USD:", extraUsd);
    ...
    function compareMultiplePurchases() private {
        ...
        if (mocGasCostRbtc > uniGasCostRbtc) {
            uint256 extraCostRbtc = mocGasCostRbtc - uniGasCostRbtc;
            uint256 extraCostUsd = (extraCostRbtc * btcPrice * 100000) / 1e18; // Multiply by 100000 for 5 decimals
            console2.log("MoC is more expensive to operate:");
            console2.log("  Extra cost in rBTC (WEI):", extraCostRbtc);
            console2.log("  Extra cost in USD: %d.%05d", extraCostUsd / 100000, extraCostUsd % 100000);
        } else if (uniGasCostRbtc > mocGasCostRbtc) {
            uint256 extraCostRbtc = uniGasCostRbtc - mocGasCostRbtc;
            uint256 extraCostUsd = (extraCostRbtc * btcPrice * 100000) / 1e18;
            console2.log("Uniswap is more expensive to operate:");
            console2.log("  Extra cost in rBTC (WEI):", extraCostRbtc);
            console2.log("  Extra cost in USD: %d.%05d", extraCostUsd / 100000, extraCostUsd % 100000);
        } else {
        ...
        if (mocRbtcPurchased > uniRbtcPurchased) {
            uint256 extraRbtc = mocRbtcPurchased - uniRbtcPurchased;
            uint256 extraUsd = (extraRbtc * btcPrice * 100000) / 1e18;
            console2.log("MoC provides better returns:");
            console2.log("  Extra rBTC gained (WEI):", extraRbtc);
            console2.log("  Extra value in USD: %d.%05d", extraUsd / 100000, extraUsd % 100000);
        } else if (uniRbtcPurchased > mocRbtcPurchased) {
            uint256 extraRbtc = uniRbtcPurchased - mocRbtcPurchased;
            uint256 extraUsd = (extraRbtc * btcPrice * 100000) / 1e18;
            console2.log("Uniswap provides better returns:");
    ...
    function printFinalComparison() private view {
        console2.log("\n=== FINAL COMPARISON ===");
        console2.log("MoC final rBTC balance (WEI):", IPurchaseRbtc(handlerMoc).getAccumulatedRbtcBalance());
        console2.log("Uniswap final rBTC balance (WEI):", IPurchaseRbtc(handlerUni).getAccumulatedRbtcBalance());
        ...
        if (mocRbtcBalance > uniRbtcBalance) {
            uint256 percentage = ((mocRbtcBalance - uniRbtcBalance) * 10000) / uniRbtcBalance;
            console2.log("Overall, MoC provided better returns by %d.%d%%", percentage / 100, percentage % 100);
        } else if (uniRbtcBalance > mocRbtcBalance) {
            uint256 percentage = ((uniRbtcBalance - mocRbtcBalance) * 10000) / mocRbtcBalance;
            console2.log("Overall, Uniswap provided better returns by %d.%d%%", percentage / 100, percentage % 100);
        } else {
